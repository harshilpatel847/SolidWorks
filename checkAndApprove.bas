...
Dim revTable As SldWorks.RevisionTableAnnotation
...
Sub main()
    ...
    If Not revTable Is Nothing Then
        revTable.Text(2, 4) = "J. SOBEL/" & UCase(Format(Date, "ddmmmyyyy"))
    End If
    
    ' Get the sheet format name for the current sheet
    formatName = swSheet.GetSheetFormatName
    ...
    boolstatus = Part.Extension.SelectByID2("DetailItem" & note(0) & "@" & formatName, "NOTE", 0, 0, 0, False, 0, Nothing, 0)
    Set chkName = Part.SelectionManager.GetSelectedObject5(1)
    chkName.SetText "J. SOBEL"
    Part.ClearSelection2 True
    
    ' Fill in the checked by date field
    boolstatus = Part.Extension.SelectByID2("DetailItem" & note(1) & "@" & formatName, "NOTE", 0, 0, 0, False, 0, Nothing, 0)
